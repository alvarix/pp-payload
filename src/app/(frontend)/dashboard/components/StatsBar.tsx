		...
		<div className="space-y-3">
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
				...
				<StatCard
					label="Feedback"
					value={feedbackCount}
					href="/admin/collections/jobs?where[testimonial][exists]=true"
...
function StatCard({
	...
	const inner = (
		<div
			className={`border ${borderColor} rounded px-3 py-1.5 bg-white shadow-sm flex items-center gap-2`}
		>
			<span className="text-lg font-bold text-gray-900 leading-none">
				{value}
			</span>
			<div className="min-w-0">
				<p className="!m-1 text-xs text-gray-600 font-medium leading-none truncate">
					{label}
				</p>
				{description && (
					<p className="!m-1 text-xs text-gray-500 leading-none truncate mt-0.5">
						{description}
					</p>
				)}
			</div>
